...
class BuffalorifleCommand extends Command {
	const OK = 1;
	...
	protected function run() {
		if ($this->check == self::OK) {
			...
			$this->game['inter_turn_reason'] = serialize(array('action' => 'buffalorifle', 'from' => $this->actualPlayer['id'], 'to' => $this->attackedPlayer['id']));
			$this->game->save();
	...
	protected function generateMessages() {
		...
		if ($this->check == self::OK) {
			$message = array(
				'text' => $this->loggedUser['username'] . ' zautocil Bufalobijkou na ' . $attackedUser['username'],
				'notToUser' => $attackedUser['id'],
			);
			$this->addMessage($message);
			
			$message = array(
				'text' => $this->loggedUser['username'] . ' na teba zautocil Bufalobijkou',
